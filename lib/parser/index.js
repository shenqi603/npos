  ...
  while (offset < raw.length) {
    ...
    if (group !== undefined && group !== null) {
      ...
      if (isNumber(group)) {
        ...
        forEach(group, function (rule, key) {
          ...
          forEach(rule, function (splitter) {
            if (isString(splitter)) {
              splitter = splitters[splitter];
            }

            if (isNumber(splitter)) {
              offset += splitter;
            } else if (isFunction(splitter)) {
...
Parser.prototype.parse = function (raw, options, cb) {
  ...
  var entries = [], entry;
  forEach(ast.tree, function (node) {
...
function findCodec(node) {
  var found = null;
  forEach(codecs, function (codec, name) {
    if (!codec) return;
    if (!Array.isArray(codec.commands) || !isFunction(codec.decode)) return;
